...
fp.csv <- '/Users/blrice/Library/CloudStorage/Dropbox/R DROPBOX/2025_CHAMELEONS/data/gps/processed_gpx_files_20260117.CSV'
...
#
icon.color_map <- ifelse(df.gps1$device == "FROG", "green", "orange")
...
map.AJJ <- leaflet() %>%
  ...
  #Adding satellite imagery
  addProviderTiles('Esri.WorldImagery') %>%
  addMarkers(data = df.gps1 %>% filter(site_code == "AJJ"), 
             lat = ~lat, lng = ~lon, popup = ~name, label = ~name, 
             #icon = makeAwesomeIcon(color = ~ifelse(device == "FROG", "green", "orange"))
             #icon = awesomeIcons(markerColor = ~ifelse(device == "FROG", "green", "orange"))
             icon = awesomeIcons(
               markerColor = ifelse(device == "FROG", "red", "blue")
             )
  )
map.AJJ
